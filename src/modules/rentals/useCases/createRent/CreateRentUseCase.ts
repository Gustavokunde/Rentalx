...
import { Rent } from "../../../rentals/entities";
import { IRentsRepository } from "../../../rentals/repositories";
...
class CreateRentUseCase implements IBaseUseCase {
  ...
  async execute({
    ...
    const openRentByUser = await this.rentsRepository.findOpenRentByUserId(
      user_id
    );
    ...
    const openRentByCar = await this.rentsRepository.findOpenRentByCarId(
      car_id
    );
